...
*
*   Copyright (C) 2019-2022 ATgroup09 (atgroup09@gmail.com)
*
...
@param  TargetIn  - target	 [STRING]
@param  MessageIn - message [STRING]
@param  ColorIn   - color   [STRING || NULL]
@return None.
*/
function writeToLog(TargetIn, MessageIn, ColorIn)
{
	if(typeof TargetIn == "string" && typeof MessageIn == "string")
	{
		var LogStamp = null;
		...
		//Log
		G_LOG.add(LogStamp, TargetIn, MessageIn, ColorIn);
	}
}
...
@return true.	[BOOLEAN]
*/
function onFuncWsLog(OptionsIn, DataIn, DataKeyIn, ValueIn, BeforeValueIn)
{
	if(typeof OptionsIn["LogTarget"] != "undefined" && G_HMI.RefreshFirstIter)
	{
		var Color    = null;
		var ColorKey = BeforeValueIn;
		
		if(typeof OptionsIn["LogColor"] == "string")
		{
			//color from string
			Color = OptionsIn["LogColor"];
		}
		else if(typeof OptionsIn["LogColor"] == "object")
		{
			//color from associative array
			if(OptionsIn["LogColor"])
			{
				if(typeof OptionsIn["LogColor"][ColorKey] == "string")
				{
					Color = OptionsIn["LogColor"][ColorKey];
				}
			}
		}
		
		writeToLog(OptionsIn["LogTarget"], ValueIn, Color);
	}
